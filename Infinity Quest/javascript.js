...
const intro = () => {
    alert(`You have woken up in a strange room. In order to leave the room, you will need strength, wisdom and courage. You remember vaguely signing up to a trial of a new game by a weird man in the street before it all went dark, promises of untold treasure and self-betterment. It seemed like a good idea… at first.`)
    alert(`The aim of the game is to collect all three infinity stones to return home. If all three stones have not been collected you will stay in this realm….TILL INFINITY!
    `)
    alert(`Please choose a door to test yourself, each will have a different challenge to complete to claim your prize`)
}
...
const getName = () => {
    ...
    if (userName != null  && userName != ''){
        ...
        alert(`Welcome, ${character.name}!`)
        intro()
    } else {
        alert(`The silent type are you? Use your words.`)
        getName()
    }
}
...
const endGame = () => {
    alert(`CONGRATULATIONS! You have collected all three stones and passed the infinity quest!`)
    alert(`You can now return home.`)
    alert(`The real treasure was the friends you made along the way.`)
    alert(`You made no friends.`)
    alert(`Enjoy your little trinkets.`)
...
const newRoom = () => {
    ...
    } else if(completedRooms.length == 2) {
        alert(`You walk out of the room with your new treasure and arrive back in the main foyer, it seems as though your door has disappeared behind you`)
...
const mathQuiz = () => {
    if(character.wisdom == 0 && wisdomAttempts == 5){
        alert(`First Question`)
        randomQuest(functionArr)
    } else if(character.wisdom < 3 && wisdomAttempts > 0){
        alert('Next Question')
        randomQuest(functionArr)
